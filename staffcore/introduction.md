...
- **Anti-swear** blocked-word list with a configurable action (`BLOCK` / `MUTE` / `WARN` / `KICK`, default `BLOCK`) and a bypass-resistant normalisation pipeline (leet substitution, diacritic strip, whitespace / punctuation strip, optional collapse-repeats). Runs on chat AND on signs, books and anvil renames. Ships with the merged LDNOOBW `en` + `de` list (~465 entries) plus a 55-entry whitelist to keep innocent words like `class`, `password` and `analysis` off the filter. The block message is shown on the action bar so it never pollutes the chat log.
- **Chat control** `/chat clear`, `/chat pause [duration]`, `/chat unpause` — network-wide chat moderation.
- **Announcements** `/announce` renders as title + chat + sound, `/broadcast` is chat-only. Both are network-wide.
- **Staff chat** `/staffchat` toggles a private staff-only channel that survives cross-server.
- **Spy** `/spy <player>` mirrors a target's public chat, private messages (both directions) and executed commands into your view. Cross-server.
- **Freeze** `/freeze <player>` locks a player in place until unfrozen. Cross-server and offline-aware (a freeze applied to an offline target takes effect on their next login).
- **Vanish** `/vanish` hides you from non-staff, silences join / quit messages and skips container open animations. Persists in the database and follows you across servers.
- **Reports** `/report <player> <reason>` filed by any player, with a per-reporter cooldown and configurable length caps. Staff review through `/reports` (network-wide list) or `/report check <player>` (per-target list). Resolved reports auto-delete after a configurable window (default 12 hours). Optional Discord webhook posts each new report as a full embed with reporter head, target thumbnail and inline metadata.
- **Inspect** `/invsee` and `/echestsee` open a target's inventory or enderchest in read-only or edit mode, cross-server aware, works for offline players too.
- **Utils** self / other variants of `/fly`, `/flyspeed`, `/speed`, `/heal`, `/feed`, `/repair`, plus local-only `/near [radius]` and `/top`. Cross-server aware and offline-aware where meaningful.
